...
if [ -z "${_consul_template_version}" ]; then
	...
	_filebeat_with_consul_template_tag=$CONSUL_TEMPLATE_VERSION
	_release_build=true
fi
...
if [ $_release_build == true ]; then
	docker build --build-arg CONSUL_TEMPLATE_VERSION=${_consul_template_version} --tag "stakater/filebeat-with-consul-template:latest"  --no-cache=true .
fi
